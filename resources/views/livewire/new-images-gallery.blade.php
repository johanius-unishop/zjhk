    ...
    <div class="row">
        @if (!is_null($images) && !$images->isEmpty())
            @foreach ($images as $image)
                <div class="col-3">
                    <figure class="figure">
                        <img widht="100%" height="100%" src="{{ $image->getUrl('thumb') }}"
        ...
        @else
            <div class="col-md-12 text-center">
                Изображение для новости не найдено
            </div>
            @if ($flag == 0)
                <div class="row">
                    <label for="photos">Изображение для новости</label>
                </div>
                <div class="row">
                    <div class="col-6">
                    ...
                    <div class="col-6">
                        <a class="btn {{ $flag == 0 ? 'btn-primary ' : 'btn-danger' }} ml-auto"
                            wire:loading.attr="disabled" wire:click="uploadFiles" href="#">Загрузить изображение</a>
                    </div>
                </div>
                <div class="row">
                    @error('photos')
