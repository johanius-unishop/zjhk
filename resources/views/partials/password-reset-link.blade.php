    ...
    <form method="POST" action="{{ route('password.email') }}" class="modal__form">
        ...
        <div class="modal__input-container">
            <div>
                ...
                <p class="notes__standard">Сообщите нам свой email, и мы отправим Вам ссылку для сброса пароля, которая позволит установить новый.</p>

            </div>
            <div>
                <label for="email" class="visually-hidden"></label>
                <input type="email" id="email-password-reset-link" name="email" class="modal__input"
                    value="{{ session('form_error_source') == 'password-reset-link' ? old('email') : '' }}" placeholder="Введите email"
